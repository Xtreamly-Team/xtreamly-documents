...
# ARIMA
# =============================================================================
def _features_arima(series, lags):
    data = pd.DataFrame(series)
    for lag in range(1, lags + 1):
        data[f'lag_{lag}'] = series.shift(freq=f'{lag*h}min')
    return data
...
for _, r_horizons in df_horizons.iloc[:].iterrows():
    for s in symbols:
        for lag in lags:
            ...
            df_s = df_s[['_time','price',f'std_{h}min']].dropna()
            df_s = df_s.rename(columns={f'std_{h}min':'y'})
            df_s['_time'] = pd.to_datetime(df_s['_time'])
            df_s.index = df_s['_time']
            df_f = _features_arima(df_s['y'], lag).dropna()
            ...
            ax2 = ax.twinx()
            ax.set_title(f"ARIMA Timeline {s} Forecast Volatility on {s}USD Price (for {h}min horizon)", pad=20)
            ...
            ax2.plot(df.index, df['y_test'], color=tailwind['emerald-500'], linewidth=1.5, alpha=.99, label=f"Actual")
            ax2.plot(df.index, df['y_pred'], color=tailwind['amber-500'], linewidth=1.5, alpha=.99, label=f"Forecast")
            ...
            fig.savefig(os.path.join('results', folder, f'ARIMA {s} {h}min horizon {lag}lag Timeline Forecast.png'), dpi=200)
            fig.clf()
            
            _style_white()
            fig, ax = plt.subplots(figsize=(16, 7))
            ax.set_title(f"ARIMA Scatter {s} Forecast on Actual Volatility (for {h}min horizon)", pad=30)
            ax.set_ylabel(f"Forcasted Volatility", labelpad=20)
            ax.set_xlabel(f"Actual Volatility", labelpad=20)
            ax.scatter(df['y_test'],df['y_pred'], color=tailwind['amber-500'],alpha=.3, s=2, label=f"Forecast")
            ...
            for spine in ax.spines.values(): spine.set_visible(False)
            fig.tight_layout(rect=[0.004, 0.004, .996, .996])
            fig.savefig(os.path.join('results', folder, f'ARIMA {s} {h}min horizon {lag}lag Scatter Forecast.png'), dpi=200)
            fig.clf()
            ...
            _style_white()
            fig, ax = plt.subplots(figsize=(16, 7))
            ax.set_title(f"ARIMA {s} Weekly Model Fit on R2 Measure (for {h}min horizon)", pad=30)
            ax.set_ylabel(f"R2 Measure",labelpad=20)
            ax.bar(df_week['Week End Time'],df_week['R2'], color=tailwind['amber-400'],alpha=.9)
            ax.set_ylim((0, 1))
            ...
            for spine in ax.spines.values(): spine.set_visible(False)
            fig.tight_layout(rect=[0.004, 0.004, .996, .996])
            fig.savefig(os.path.join('results', folder, f'ARIMA {s} {h}min horizon {lag}lag Weekly R2.png'), dpi=200)
            fig.clf()
            
            _style_white()
            fig, ax = plt.subplots(figsize=(16, 7))
            ax.set_title(f"ARIMA {s} Weekly Model Fit on Correlation Measure (for {h}min horizon)", pad=30)
            ax.set_ylabel(f"R2 Measure",labelpad=20)
            ax.bar(df_week['Week End Time'],df_week['Corr.'], color=tailwind['amber-300'],alpha=.9)
            ax.set_ylim((0, 1))
            ax.yaxis.set_major_formatter(mtick.FuncFormatter(lambda x, pos: '{:,.0f}%'.format(100*x)))            
            ax.set_yticks(ax.get_yticks())
            ax.set_xticks(ax.get_xticks())
            ...
            for spine in ax.spines.values(): spine.set_visible(False)
            fig.tight_layout(rect=[0.004, 0.004, .996, .996])
            fig.savefig(os.path.join('results', folder, f'ARIMA {s} {h}min horizon {lag}lag Weekly Corr.png'), dpi=200)
            fig.clf()    
